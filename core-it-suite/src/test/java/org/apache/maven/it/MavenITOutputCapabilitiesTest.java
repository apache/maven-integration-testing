...
class MavenITOutputCapabilitiesTest extends AbstractMavenIntegrationTestCase {
    ...
    void injectsCapabilitiesIntoLegacyPluginsInParallelReactor() throws Exception {
        File directory = ResourceExtractor.simpleExtractResources(getClass(), "/output-capabilities");
        Verifier plugin = newVerifier(new File(directory, "plugin").getPath(), "remote");
        plugin.addCliArgument("install");
        plugin.execute();
        plugin.verifyErrorFreeLog();
        Verifier legacyPlugin = newVerifier(new File(directory, "legacy-plugin").getPath(), "remote");
        ...
        for (String color : new String[] {"never", "always"}) {
            ...
            consumer.addCliArgument("-l");
            consumer.addCliArgument("build.log");
            ...
            for (String prefix : new String[] {"", "child/"}) {
                ...
                // Maven 3 opens -l output with the JVM default charset.
                assertEquals("FILE", properties.getProperty("destination"));
                assertEquals(properties.getProperty("defaultEncoding"), properties.getProperty("encoding"));
            }
        }
    }
}
